...
- **Auto-Pilot** (🟢) — **closed-book and fail-OPEN**; **never learns**. The Student answers **only** from the approved Classroom: a Classroom match → a **grounded** reply auto-sends and the conversation stays green; **no match → a graceful out-of-scope ack and the conversation stays green**; a responder/LLM error → a graceful fallback ack (**never silent**) that counts toward the breaker. **Circuit breaker**: the 3rd *consecutive* fallback (or >3 in a rolling 2-min window) sends a final ack and steps the conversation **GREEN→BLUE** (`engagementModeOverride='manual'`, not auto-cleared — a human re-enables once it's trained). Compliance/opt-out are re-checked at send (`evaluateAutoPilotTurn` gate table); a human can step in at any time.

The auto-send gate (`artifacts/api-server/src/lib/engagementPolicy.ts`, `evaluateAutoPilotTurn`) is **closed-book and fail-OPEN** — category **no longer blocks** auto-send. Per inbound (first match wins): a Classroom match → auto-send a **grounded** answer (stay green, reset the fallback run); **no match → a graceful out-of-scope ack and the conversation stays green**; a responder/LLM error → a graceful fallback ack (**never silent**) that counts toward the breaker. The only **hard** guard is outbound **compliance/opt-out** (re-checked at send); the **breaker** steps a conversation GREEN→BLUE on the 3rd *consecutive* fallback (or >3 in a rolling 2-min window).

**Learning rule:** **no engagement mode learns at runtime** (as of 2026-06-27). Auto-Pilot is **closed-book** (no Professor, no fact persistence), Co-Pilot only drafts for a human, and Manual is off — **no inbound path persists facts**. Facts enter the Classroom **only** via a human-driven push (Human + Professor, or Brain + Human, through the Conductor).
