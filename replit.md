...
## User Preferences
- GitHub remote (primary, as of 2026-06-15): `https://github.com/Tableicity/Textitie_June.git` — PAT stored in the `GITHUB_TABLEICITY` secret (account **Tableicity**, which owns the repo). For one-off shell pushes use:
  `git push "https://Tableicity:${GITHUB_TABLEICITY}@github.com/Tableicity/Textitie_June.git" main`
  - **Why the move:** the old repo `TransferAgent/textitie` is owned by a *different* GitHub account than the one Replit's git connection is bound to (Tableicity / info@tableicty.com). Replit-as-Tableicity got a 403 pushing to a TransferAgent repo, and the `replit-git-askpass` helper hung instead of erroring — so both the GUI "Push" and plain `git push origin` froze. **Use the shell command above (inline Tableicity token) to push — it bypasses the broken helper and works every time.** Always redact the token in output (`sed "s#${GITHUB_TABLEICITY}#***#g"`) and use `GIT_TERMINAL_PROMPT=0`.
  - Old backup remote: `https://github.com/TransferAgent/textitie.git` (PAT `GITHUB_TEXTITIE`, account TransferAgent) holds history through tip `b95b9d6`.
  - **Verified 2026-06-15:** `origin` was repointed at the new repo (`git remote set-url origin "https://github.com/Tableicity/Textitie_June.git"`) and the live tip matches local. BUT plain `git push origin main` and the Replit GUI "Push" button **still hang** (exit 124) — the cause is Replit's `replit-git-askpass` credential helper hanging regardless of which repo `origin` points to (a broken Replit↔GitHub connection, not a repo problem). The inline-token shell command above is the reliable push path. To attempt fixing the GUI itself: fully disconnect the GitHub connection in Replit, then reconnect while signed into GitHub as **Tableicity**; if askpass still hangs after a clean reconnect, it's a Replit-side bug → contact Replit support with the evidence (`replit-git-askpass` exits 124). **Confirmed 2026-06-15:** a full GitHub logout/login (commit-author identity now matches the Tableicity repo owner) did NOT fix it — askpass and plain `git push origin` still hang (exit 124). The hang is therefore a Replit platform bug, not an identity/account issue. Use the inline-token shell push; escalate the GUI hang to Replit support.
