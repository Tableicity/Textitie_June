...
- GitHub remote (primary, as of 2026-06-15): `https://github.com/Tableicity/Textitie_June.git` — PAT stored in the `GITHUB_TABLEICITY` secret (account **Tableicity**, which owns the repo). For one-off shell pushes use:
  ...
  - To make the Replit GUI "Push" button target the new repo, point origin at it (git-config write — run it yourself; the agent is blocked from git-config writes): `git remote set-url origin "https://github.com/Tableicity/Textitie_June.git"`
